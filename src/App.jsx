...
function App() {
	...
	const [isPointDescriptionSubmitted, setIsPointDescriptionSubmitted] =
		useState(false);
	const [showPolylineForm, setShowPolylineForm] = useState(false);
	const [isPolylineDescriptionSubmitted, setIsPolylineDescriptionSubmitted] =
		useState(false);
	...
	useEffect(() => {
		if (mapDiv.current) {
			...
			function pointPopupContent(feature) {
				...
				if (attributes.utilityAttachments == null) {
					...
					utilityAttachmentsHTML = JSON.parse(
						...
						.map((attachment) => {
							return `<li>
				      <p><strong>Utility Owner:</strong> ${attachment.utilityOwner || ''}
				      <strong>Splice Case:</strong> ${attachment.spliceCase || ''}</p>
				    </li>`;
						})
						.join('');
				}
				...
				div.innerHTML = `
					   <div>
					     ...
					     <p><strong>Pole Owner:</strong> ${attributes.poleOwner || ''}</p>
					     <p><strong>Power Phase:</strong> ${attributes.powerPhase || ''}</p>
			...
			const polylineLayer = new FeatureLayer({
				...
				popupTemplate: {
					title: 'Polyline',
					actions: [polylineEditThisAction], // Define actions if needed
